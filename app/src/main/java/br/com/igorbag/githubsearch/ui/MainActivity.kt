...
class MainActivity : AppCompatActivity() {
    ...
    fun setupView() {
        ...
        userNotFoundText = findViewById(R.id.tv_user_not_found)
        userNotFoundImage = findViewById(R.id.iv_user_not_found)
        ...
        if (user != "") {
            ...
            githubApi.getAllRepositoriesByUser(user).enqueue(object : Callback<List<Repository>> {
                ...
                override fun onFailure(call: Call<List<Repository>>, t: Throwable) {
                    Toast.makeText(
                        applicationContext, "deu ruim", Toast.LENGTH_LONG
                    ).show()
                    progressBar.isVisible = false
    ...
    fun setupList(list: List<Repository>) {
        if (list.isEmpty()) {
            resultLabel.isVisible = false
            listaRepositories.isVisible = false
            ...
            adapter.repositoryItemListener = {
                openBrowser(it.htmlUrl)
            }
            listaRepositories.adapter = adapter
            listaRepositories.addItemDecoration(
    ...
    fun shareRepositoryLink(repository: Repository) {
        val sendIntent: Intent = Intent().apply {
            ...
            putExtra(
                Intent.EXTRA_TEXT,
                "Confira o repositório ${repository.name} de ${nomeUsuario.text}: ${repository.htmlUrl}"
            )
            type = "text/plain"
        }
